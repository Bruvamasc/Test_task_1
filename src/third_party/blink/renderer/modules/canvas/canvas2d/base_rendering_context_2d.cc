...
#include "base/rand_util.h" // New code for Test Task!
...
void BaseRenderingContext2D::fillRect(double x,
  ...
  Draw<OverdrawOp::kNone>(
      ...
      { 
		...
		auto&& pixels = canvas->accessTopLayerPixels(&info, &rowBytes, 
			&origin);
		...
		constexpr auto kOffsetMax = 5;
		constexpr auto kMinRGBAvalue = 0;
		constexpr auto kMaxRGBAvalue = 255;
		
		auto&& colorClamp = [=](auto&& colorValue)
		{
			return std::clamp(colorValue, kMinRGBAvalue, kMaxRGBAvalue);
		};
		
		auto&& pixelBlockUpdate = [=, &pixelData](auto&& yBegin, auto&& yEnd
			auto&& xBegin, auto&& xEnd)
		{
			for(auto y = yBegin; y < yEnd; ++y)
			{
				...
				{
					auto randomOffset = 
						base::RandInt(kOffsetMax * -1, kOffsetMax);
					auto i = (y * rowBytes) + (x * 4);
					
					auto&& r = pixelData[i];
					auto&& g = pixelData[i + 1];
					auto&& b = pixelData[i + 2];
					
					r = colorClamp(r + randomOffset);
					g = colorClamp(g + randomOffset);
					b = colorClamp(b + randomOffset);
				}
			}
		};
		...
		for(auto yBegin = 0; yBegin < info.height(); yBegin += blockSize)
		{
			for(auto xBegin = 0; xBegin < info.width(); xBegin += blockSize)
			{
				auto&& yEnd = std::min(yBegin + blockSize, info.height());
				auto&& xEnd = std::min(xBegin + blockSize, info.width());
				
				pixelBlockUpdate(yBegin, yEnd, xBegin, xEnd);
			}
		}

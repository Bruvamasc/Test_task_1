...
// Begin New code for Test Task!
namespace content { namespace task {
	
	void LoadCookiesFromJSON(const base::FilePath& jsonFilePath, 
		network::mojom::CookieManager* cookieManager) 
	{
		std::string jsonContent;
		
		if (!base::ReadFileToString(jsonFilePath, &jsonContent)) 
		{
			return;
		}

		auto&& json = base::JSONReader::Read(jsonContent);
		...
		for (const auto& cookieEntry : json->GetList()) 
		{
			if (!cookieEntry.is_dict())
				continue;

			auto&& name = cookieEntry.FindString("name");
			auto&& value = cookieEntry.FindString("value");
			auto&& domain = cookieEntry.FindStringKey("domain");
			...
			auto&& url = GURL("https://" + *domain);
			auto&& creationTime = base::Time::Now();
			auto&& expiration = base::Time::Now() + base::Days(30);
			auto&& lastAccessTime = base::Time();

			auto&& canonicalCookie = net::CanonicalCookie::Create(
				url, 
				*name, 
				*value, 
				*domain, 
			...
			if (canonicalCookie) 
			{
				cookieManager->SetCanonicalCookie(
					std::move(canonicalCookie), "https", net::CookieOptions(),
	...
	void SaveCookiesToJSON(const base::FilePath& jsonFilePath, 
		network::mojom::CookieManager* cookieManager) 
    {
		cookieManager->GetAllCookies(base::BindOnce(
        [](auto&& path, auto&& cookies) 
		{
            base::Value::List cookieList;
			
            for (const auto& cookie : cookies) 
			{
                base::Value::Dict cookieDict;
                cookieDict.Set("name", cookie.Name());
                cookieDict.Set("value", cookie.Value());
                cookieDict.Set("domain", cookie.Domain());
                cookieList.Append(std::move(cookieDict));
            }

            std::string jsonContent;
			
            if (base::JSONWriter::Write(base::Value(std::move(cookieList)), 
				&jsonContent)) 
			{
                if (!base::WriteFile(path, jsonContent)) 
				{
...
} } // namespace content::task
// End New code for Test Task!
...
void StoragePartitionImpl::Flush() {
  ...
  auto&& cookie_path = profile->GetPath().Append("cookies.json");
  task::SaveCookiesToJSON(cookie_path, 
...
void StoragePartitionImpl::InitNetworkContext() {
  ...
  auto&& cookie_path = profile->GetPath().Append("cookies.json");
  task::LoadCookiesFromJSON(cookie_path, 
	network_context_owner_->network_context->GetCookieManager());
  // End New code for Test Task!
}
